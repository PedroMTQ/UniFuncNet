...
class Counter():
    ...
    def get_most_common_string(self):
        if self.get_strings():
            try:    return next(self.get_strings())
            except: return None
        else: return None

    def remove_string(self,string_to_remove):
        if string_to_remove in self.possible_strings: self.possible_strings.pop(string_to_remove)

    def get_possible_strings(self):
        if None in self.possible_strings and len(self.possible_strings)== 1: return {}
