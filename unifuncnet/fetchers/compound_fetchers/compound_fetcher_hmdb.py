...
class CompoundFetcherHmdb(CompoundFetcher):
    ...
    def get_compound_hmdb(self):
        ...
        if smiles:
            smiles = smiles.findNext().text
        res['smiles'] = smiles
        inchi_key = soup.find('th', string='InChI Key')
        if inchi_key:
            inchi_key = remove_inchi_key_equal(inchi_key.findNext().text)
        res['inchi_key'] = inchi_key
        inchi = soup.find('th', string='InChI Identifier')
        if inchi:
            inchi = remove_inchi_key_equal(inchi.findNext().text)
        res['inchi'] = inchi
        synonyms = []
...
if __name__ == '__main__':
    c = CompoundFetcherHmdb('HMDB00131759')
    c = CompoundFetcherHmdb('HMDB00131836')
    c = CompoundFetcherHmdb('HMDB00131837')
