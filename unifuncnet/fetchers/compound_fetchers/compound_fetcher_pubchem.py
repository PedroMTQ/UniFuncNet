...
class CompoundFetcherPubChem(CompoundFetcher):
    ...
    def convert_inchi_to_pubchem_id(self):
        ...
        if self.db == 'inchi_key':
            webpage = self.get_with_fetcher('https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/inchikey/cids/JSON/',
                                            data={'inchikey': f'{self.compound_id}'},
            ...
            webpage = self.get_with_fetcher('https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/inchi/cids/JSON/',
                                            data={'inchi': f'InChI={self.compound_id}'},
                                            original_response=True)
            if webpage:
                json_result = webpage.json()
        if json_result:
            ...
            if self.db == 'inchi_key':
                webpage = self.get_with_fetcher(
                    'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/inchikey/sids/JSON/',
...
if __name__ == '__main__':
    search = CompoundFetcherPubChem('HONVBDHDLJUKLX-UHFFFAOYSA-N', 'inchi_key')
    search = CompoundFetcherPubChem('962', 'pubchem_cid')
    search = CompoundFetcherPubChem('1S/C5H7NO2/c6-5-3(7)1-2-4(5)8/h5H,1-2,6H2', 'inchi')
