...
class ReactionFetcherKegg(ReactionFetcher):
    ...
    def set_convergence_args(self, extra_args):
        if 'compound' in extra_args:
            self.convergence_args['compound'] = extra_args['compound']
        else:
            self.convergence_args['compound'] = None
        # For convergence
    ...
    def get_reactions_KEGG(self):
        ...
        if rn_info_rn_soup:
            ...
            if self.check_correct_api_kegg_search(rn_info['rn_comp_id'], self.convergence_args['compound']):
                ...
                if rn_info['rn_instances']:
                    res['reaction_with_instances'] = rn_info['rn_instances']
                else:
                    res['rn_with_ids'] = [rn_info['rn'], rn_info['rn_comp_id'], 'kegg']
                return Reaction(res)
            else:
...
if __name__ == '__main__':
    rn_search = ReactionFetcherKegg('R00018')
