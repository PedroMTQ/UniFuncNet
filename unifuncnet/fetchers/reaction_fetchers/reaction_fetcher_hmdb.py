...
class ReactionFetcherHmdb(ReactionFetcher):
    def __init__(self, reaction_id, extra_args={}, memory_storage=None):
        ReactionFetcher.__init__(self, reaction_id=reaction_id, memory_storage=memory_storage)
        self.db = 'hmdb'
    ...
    def set_convergence_args(self, extra_args):
        ...
        if 'proteins_list' in extra_args:
            self.convergence_args['proteins_list'] = extra_args['proteins_list']
        else:
            self.convergence_args['proteins_list'] = []
        if 'protein_id' in extra_args: self.convergence_args['proteins_list'] = self.convergence_args[
            'proteins_list'].append(extra_args['protein_id'])
        if 'protein_soup' in extra_args:
            self.convergence_args['protein_soup'] = extra_args['protein_soup']
        else:
            self.convergence_args['protein_soup'] = None
        # args for getting reaction
        if 'cpd_id' in extra_args:
            self.convergence_args['cpd_id'] = extra_args['cpd_id']
        else:
            self.convergence_args['cpd_id'] = None
    ...
    def get_details_reaction_compound_page(self, details_source):
        details_list = [[th.get_text(), ''] for th in details_unifuncnet.find_all("dt")]
        details_text = details_unifuncnet.find_all("dd")
    ...
    def get_reaction_HMDB(self):
        ...
        if rn_with_instances:
            rn_info['reaction_with_instances'] = rn_with_instances
        else:
            rn_info['rn_with_ids'] = [reaction_str, rn_with_ids, 'hmdb']
