...
from unifuncnet.biological_components.utils.counter import Counter
from unifuncnet.biological_components.utils.item_set import ItemSet
from unifuncnet.biological_components.utils.synonyms import Synonyms
from unifuncnet.biological_components.utils.unique_details import (get_details_list,
                                                                   generate_details_dict,
                                                                   get_unique_details)
from unifuncnet.utils.util import (find_sign,
                                   uniform_sign,
                                   get_instance_type,
                                   fix_html_sign,
                                   SCRAPPABLE_DBS,
...
class BaseComponent:
    ...
    def get_unique_details(self, remove_from_list=[], append_to_list=[]):
        return get_unique_details(bio_instance=self,
                                  remove_from_list=remove_from_list,
                                  append_to_list=append_to_list)

    def get_details_list(self, extra_instances=[],
                         remove_from_list=[],
                         append_to_list=[]):
        bio_instances = []
        ...
        return get_details_list(bio_instances=bio_instances,
                                remove_from_list=remove_from_list,
                                append_to_list=append_to_list)
    ...
    def export_all_info(self):
        ...
        for d_key in self.get_details_list(remove_from_list=['internal_id']):
            if d_key == 'reaction_with_instances':
                ...
                if details:
                    for d in details:
                        if is_ec(d, 4) and d_key != 'enzyme_ec':
                            pass
                        else:
    ...
    def set_detail(self, detail_type, detail, converged_in=None):
        if detail is not None:
            if detail_type == 'reaction_with_instances':
                self.set_reaction_with_instances(detail)
            elif detail_type == 'rn_with_ids':
                self.set_rn_with_ids(detail)
            else:
                res = {}
                ...
                for to_add in res:
                    ...
                    if detail_type == 'synonyms':
                        self.set_synonyms(to_add)
                    elif detail_type.endswith('_instances'):
                        self.set_instances(instances_type=detail_type, instances_to_add=to_add,
                                           converged_in=converged_in)
                    elif detail_type == 'reaction_str':
                        self.set_reaction(to_add)
                    elif detail_type == 'pathways':
                        self.set_pathways(to_add)
    ...
    def get_detail(self, detail_type, all_possible=False, ignore_detail=None, return_instance=False,
                   return_convergence=False):
        if detail_type == ignore_detail:
            return None
        if detail_type == 'reaction_with_instances':
            return self.get_reaction_with_instances()
        elif detail_type == 'rn_with_ids':
            return self.get_rn_with_ids()
        elif detail_type == 'synonyms':
            if not all_possible:
                return self.get_synonyms()
            else:
                return self.get_possible_synonyms()
        elif detail_type == 'reaction_str':
            if all_possible:
                return self.get_possible_str_reaction()
            else:
                return self.get_reaction()
        elif detail_type == 'pathways':
            return self.get_pathways()
        elif detail_type.endswith('_instances'):
            return self.get_instances(instances_type=detail_type,
                                      return_instance=return_instance,
                                      return_convergence=return_convergence)
        else:
            if all_possible:
                return self.get_possible_ids(detail_type)
            else:
                return self.get_id(detail_type)

    # Deleting information
    def remove_detail(self, detail_type, detail_to_remove):
        if detail_type == 'rn_with_ids':
            self.remove_rn_with_ids(detail_to_remove)
        elif detail_type == 'synonyms':
            self.remove_synonym(detail_to_remove)
        elif detail_type == 'reaction_str':
            self.remove_reaction(detail_to_remove)
        elif detail_type == 'pathways':
            self.remove_pathways(detail_to_remove)
        elif detail_type.endswith('_instances'):
            return self.remove_detail_type_instances(self, detail_type, detail_to_remove)
        else:
            self.remove_id(db=detail_type, id_to_remove=detail_to_remove)

    def get_id(self, db):
        if not db.endswith('_id'):
            db_str = db + '_id'
        else:
            db_str = db
    ...
    # Setting information
    # Databases IDs are actually instances of class Counter, which will define a compound's ID
    # as the most frequent one from  a list of redundant IDs
    ...
    def set_instances(self, instances_type, instances_to_add, converged_in=None):
        if instances_type in self.instances:
            ...
            self.instances[instances_type] = ItemSet(instances_to_add, converged_in)

    def set_pathways(self, pathways):
        if hasattr(self, 'pathways'):
            self.pathways.set_item_set(pathways)
        else:
            self.pathways = ItemSet(pathways)
    ...
    def is_match_absolute(self, instance_2):
        if self.check_if_same_instances(instance_2):
            return True
        counter_max = 0
        counter_match = 0
        for detail_type in self.get_unique_details():
            if detail_type == 'reaction_with_instances':
                ...
                for rn_key in rn_inst_1_dict:
                    self_l.extend(rn_inst_1_dict[rn_key])
                instance_2_l = []
                rn_inst_2_dict = instance_2.get_detail(detail_type)
                for rn_key in rn_inst_2_dict:
                    instance_2_l.extend(rn_inst_2_dict[rn_key])
                if self_l != instance_2_l:
                    return False
            else:
    ...
    def is_match_instances(self, instance_2, threshold_for_match=None, no_penalties=False):
        ...
        if not threshold_for_match:
            if node_type == 'Compound':
                threshold_for_match = 3
            elif node_type == 'Gene':
                threshold_for_match = 2
            elif node_type == 'Protein':
                threshold_for_match = 2
            elif node_type == 'Reaction':
                threshold_for_match = 2
        if c >= threshold_for_match:
            return True
        else:
            return False
...
if __name__ == '__main__':
    bc = BaseComponent({'kegg_ko': 'KOOOO1'})
