        ...
        for detail_type in self.get_unique_details(append_to_list=['synonyms']):
            if detail_type == 'synonyms':
                if self.get_detail(detail_type, all_possible=True):
                    if list_has_common_items(self.get_detail(detail_type, all_possible=True),
                                             instance_2.get_detail(detail_type, all_possible=True)):
                        c += 0.5
            else:
                c += score_match_possible_ids(self.get_detail(detail_type, all_possible=True),
                                              instance_2.get_detail(detail_type, all_possible=True))
        if test_match_possible_ids(self.get_detail('enzyme_ec', all_possible=True),
                                   instance_2.get_detail('enzyme_ec', all_possible=True)):
            if len(self.get_detail('enzyme_ec', all_possible=True)) == 1 and len(
                    instance_2.get_detail('enzyme_ec', all_possible=True)) == 1:
                c += 1
        return c

    def unite_instances_bio_specific(self, instance_2):
        for detail_type in self.get_details_list(extra_instances=instance_2):
            if detail_type == 'pathways':
                self.set_detail(detail_type, instance_2.get_detail(detail_type, all_possible=True))
