# UniFuncNet modules
from unifuncnet.Utils.util import (l_rn_ids_to_str,
                                   unite_possible_ids,
...
class Reaction(BaseComponent):
    ...
    def is_match_reaction_instances(self, reaction_instance_2):
        ...
        for rn_key in rn_inst_1_dict:
            rn_inst_1.extend(rn_inst_1_dict[rn_key])

        rn_inst_2 = []
        rn_inst_2_dict = reaction_instance_2.get_detail('reaction_with_instances')
        for rn_key in rn_inst_2_dict:
            rn_inst_2.extend(rn_inst_2_dict[rn_key])

        if not rn_inst_1 or not rn_inst_2:
            return False
        if len(rn_inst_1) != len(rn_inst_2):
            return False
        rn_inst_1_list = []
        rn_inst_2_list = []
