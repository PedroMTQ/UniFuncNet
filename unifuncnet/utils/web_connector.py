...
from unifuncnet.utils.util import find_path, UniFuncNet_FOLDER, SPLITTER,xstr
...
class WebConnector:
    ...
    def get_randrange_retry_timer(self, denominator=4):
        allowed_range = [self.get_retry_timer() - self.get_retry_timer() / denominator,
                         self.get_retry_timer() + self.get_retry_timer() / denominator]
    ...
    def dynamic_politeness_timer(self, req_start, req_stop, c):
        ...
        multiplier = 1
        if ratio in range(25, 50):
            multiplier = 2
        elif ratio in range(50, 75):
            multiplier = 5
    ...
    def allow_request(self):
        ...
        if time.time() - self.get_politeness_timer() - self.get_politeness_timer() * (
                randrange(0, 100) / 100) >= self.get_request_time():
    ...
    def try_until_catch(self, url, data=None, original_response=False, exceptional_try_limit=None):
        ...
        error = False
        c = 0
        if exceptional_try_limit:
            try_limit = exceptional_try_limit
        else:
            try_limit = self.try_limit
        while c <= try_limit:
            ...
            # previous status are the most common, but there can be exceptions
                ...
                if not self.omit_errors(): print(
                    'Server error (requests-try ' + str(c) + ') while getting ' + url + ' , retrying again in ' + str(
                        randrange_retry_timer) + ' seconds.')
