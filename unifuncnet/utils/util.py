...
def find_path(to_search_name, to_search='file', end_dir='UniFuncNet', parent_directory=None):
    ...
    if not isinstance(reaction_with_ids, list):
        ...
        if len(complete_l) != len(complete_l_id):
            ...
            if len(temp_complete_l) != len(complete_l_id):
                temp_complete_l, temp_len_sub = get_stoichiometry_reaction(reaction, ignore_numbers,
                                                                           split_plus_str='+ ')
            if len(temp_complete_l) == len(complete_l_id):
...
def get_id_or_str(rn_cpd):
    if not isinstance(rn_cpd, str):
        return rn_cpd.get_most_common_synonym()
    else:
        return rn_cpd
...
def test_match_possible_ids(possible_ids_1, possible_ids_2):
    ...
    if isinstance(possible_ids_1, str) or isinstance(possible_ids_1, int):
        ...
        temp_possible_ids_1 = possible_ids_1
    ...
    if isinstance(possible_ids_2, str) or isinstance(possible_ids_2, int):
        ...
        temp_possible_ids_2 = possible_ids_2
