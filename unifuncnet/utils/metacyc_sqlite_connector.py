from unifuncnet.Utils.util import SPLITTER, RESOURCES_FOLDER, strip_tags, remove_inchi_key_equal, check_all_resources
import re
import os
import sqlite3
...
class MetacycSqliteConnector():
        ...
        for db in self.db_headers[target_table]:
            if db in row_dict:
                if isinstance(row_dict[db], str):
                    ...
                    to_add = self.info_splitter.join(row_dict[db])
                res.append(to_add)
            else:
                res.append(None)
        return res
    ...
    def fetch_metacyc_intermediate_rxn_ids(self, wanted_id):
        ...
        if 'METACYCPRT' in temp:
            res['protein_ids'] = temp['METACYCPRT']
        if 'METACYCRXN' in temp:
            res['reaction_ids'] = temp['METACYCRXN']
        return res
    ...
    def test_db(self):
        for generator, gen_type in [[self.parse_compounds(), 'compound'],
                                    [self.parse_reactions(), 'reaction'],
                                    [self.parse_proteins(), 'protein'],
                                    [self.parse_genes(), 'gene']]:
            for row_dict in generator:
...
if __name__ == '__main__':
    ...
    print(s.fetch_metacyc_id_info('RXN-10728', 'reaction'))
    print(s.fetch_metacyc_id_info('RXN-14064', 'reaction'))
