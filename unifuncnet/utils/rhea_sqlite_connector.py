from unifuncnet.Utils.util import (get_stoichiometry,
                                   standardize_reaction_str,
                                   SPLITTER,
                                   download_file_ftp,
                                   gunzip,RESOURCES_FOLDER)
import re
import os
import sqlite3


class RheaSqliteConnector:
    def __init__(self):
        ...
        with open(rhea2xrefs_path) as file:
            ...
            while line:
                ...
                if line:
                    ...
                    if master_id not in rhea2ids:
                        rhea2ids[master_id] = {}
                    if db_type == 'EC':
                        db_type = 'enzyme_ec'
                    elif db_type == 'METACYC':
                        db_type = 'metacyc'
                    elif db_type == 'ECOCYC':
                        db_type = 'metacyc'
                    elif db_type == 'KEGG_REACTION':
                        db_type = 'kegg'
                        ...
                        db_type = None
                    if db_type:
                        if db_type not in rhea2ids[master_id]:
                            rhea2ids[master_id][db_type] = set()
    ...
    def fetch_rhea_from_id(self, id_type, input_id):
        ...
        if id_type == 'enzyme_ec':
            id_type_sql = 'ENZYMEEC'
        elif id_type in ['metacyc', 'kegg', 'uniprot']:
            id_type_sql = id_type.upper()
        if not id_type_sql or not input_id:
            return res
