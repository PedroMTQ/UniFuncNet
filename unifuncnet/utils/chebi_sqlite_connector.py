...
class ChebiSqliteConnector():
                ...
                while line:
                    ...
                    if line.startswith('id: CHEBI:'):
                        ...
                        if len(line) == 4:
                            ...
                            if link_type.endswith('formula'):
                                link_type = 'chemical_formula'
                            elif link_type.endswith('smiles'):
                                link_type = 'smiles'
                            elif link_type.endswith('inchikey'):
                                link_type = 'inchi_key'
                            else:
                                link_type = None
                            if link_type:
    ...
    def chebi_create_db(self):
        ...
        self.chebi_start_sqlite_cursor()
        create_table_command = '''  CREATE TABLE CHEBI2OTHERS 
                                    (CHEBI INTEGER,
                                     DATABASE TEXT, 
                                     ALTID  TEXT )'''
    ...
    def store_chebi2others(self):
        ...
        for table_chunk in generator_insert:
            insert_command = 'INSERT INTO CHEBI2OTHERS (CHEBI, DATABASE, ALTID) values (?,?,?)'
    ...
    def fetch_chebi_alt_id_info(self, chebi_id):
        ...
        if len(main_chebi_id) > 1:
            raise Exception
        elif not main_chebi_id:
            return res
        else:
