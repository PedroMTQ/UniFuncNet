...
from unifuncnet.fetchers.protein_fetchers.protein_fetcher_metacyc import ProteinFetcherMetacyc
from unifuncnet.fetchers.protein_fetchers.protein_fetcher_kegg import ProteinFetcherKegg
from unifuncnet.fetchers.protein_fetchers.protein_fetcher_hmdb import ProteinFetcherHmdb
from unifuncnet.fetchers.protein_fetchers.protein_fetcher_rhea import ProteinFetcherRhea
from unifuncnet.utils.util import is_ec

from types import GeneratorType as generator


class ProteinSearcher(GlobalSearcher):
    def __init__(self,
                 memory_storage=None,
                 search_mode='',
                 db_name=None,
                 ...
                 output_folder=None,
                 politeness_timer=10):
        GlobalSearcher.__init__(self,
                                memory_storage,
                                search_mode,
                                db_name=db_name,
                                wanted_org_kegg_codes=wanted_org_kegg_codes,
    ...
    def select_fetcher(self, db, query_id, extra_args):
        if db in SCRAPPABLE_DBS and not self.check_already_searched_memory(db, query_id):
            if db == 'metacyc':
                return ProteinFetcherMetacyc(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif 'kegg' in db:
                return ProteinFetcherKegg(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif db == 'hmdb':
                return ProteinFetcherHmdb(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif db == 'rhea':
                return ProteinFetcherRhea(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            else:
    ...
    def run_searcher(self, bio_query, bio_db, convergence_search=False, extra_args={}):
        ...
        while args_to_search:
            ...
            for s_id_arg in id_arg:
                if db_arg == 'enzyme_ec':
                    temp_inst = self.get_protein_from_ec(s_id_arg, convergence_search=convergence_search)
                elif db_arg == 'uniprot':
                    temp_inst = self.get_proteins_from_uniprot(s_id_arg, convergence_search=convergence_search)
                elif db_arg == 'kegg_ko':
                    ...
                    for ec in ko_ecs:
                        ...
                        if temp_inst:
                            ...
                            for p in temp_inst:
                                if p:
                                    p.set_detail('kegg_ko', s_id_arg)
                else:
                    if s_id_arg and not self.check_already_searched_memory(dict_input_key=db_arg,
                                                                           dict_input_value=s_id_arg):
                        temp_inst = self.find_protein(db=db_arg, query_id=s_id_arg,
                                                      convergence_search=convergence_search, extra_args=extra_args)
                self.add_to_already_tried_to_search(db_arg, s_id_arg)
                if temp_inst:
                    self.add_to_args_to_search(temp_inst, args_to_search)
    ...
    def add_to_args_to_search(self, protein_instance_or_list, args_to_search):
        ...
        for protein_instance in protein_instance_or_list:
            if protein_instance:
                ...
                if id_to_add and not self.check_already_searched_memory(dict_input_key='uniprot',
                                                                        dict_input_value=id_to_add):
                    args_to_search.append(['uniprot', id_to_add])

                id_to_add = protein_instance.get_detail('enzyme_ec')
                if id_to_add and not self.check_already_searched_memory(dict_input_key='enzyme_ec',
                                                                        dict_input_value=id_to_add):
                    args_to_search.append(['enzyme_ec', id_to_add])

                id_to_add = protein_instance.get_detail('enzyme_ec')
                if id_to_add and not self.check_already_searched_memory(dict_input_key='kegg',
                                                                        dict_input_value=id_to_add):
                    args_to_search.append(['kegg', id_to_add])

                id_to_add = protein_instance.get_detail('metacyc')
                if id_to_add and not self.check_already_searched_memory(dict_input_key='metacyc',
                                                                        dict_input_value=id_to_add):
                    args_to_search.append(['metacyc', id_to_add])

                id_to_add = protein_instance.get_detail('hmdb')
                if id_to_add and not self.check_already_searched_memory(dict_input_key='hmdb',
                                                                        dict_input_value=id_to_add):
    ...
    def get_proteins_from_uniprot(self, uniprot_id, convergence_search=False, extra_args={}):
        ...
        for db in SCRAPPABLE_DBS:
            if uniprot_id and not self.check_already_searched_memory(dict_input_key=f'{db}_uniprot',
                                                                     dict_input_value=uniprot_id):
    ...
    def get_protein_from_ec(self, enzyme_ec, convergence_search=False, extra_args={}):
        ...
        if is_ec(enzyme_ec, 4):
            ...
            if enzyme_ec and not self.check_already_searched_memory(dict_input_key='metacyc',
                                                                    dict_input_value=enzyme_ec):
                metacyc_prot = self.find_protein(db='metacyc', query_id=enzyme_ec,
                                                 convergence_search=convergence_search, extra_args=extra_args)
                self.add_to_already_tried_to_search('metacyc', enzyme_ec)

        if isinstance(kegg_prot, list):
            res.extend(kegg_prot)
        else:
            if kegg_prot:
                res.append(kegg_prot)
        if isinstance(metacyc_prot, list):
            res.extend(metacyc_prot)
        else:
            if metacyc_prot:
                res.append(metacyc_prot)

        for p in res:
            if not protein_instance:
                protein_instance = p
            if p:
    ...
    def get_db_id_from_uniprot(self, db, uniprot_id=None):
        res = []
        if 'uniprot' not in SCRAPPABLE_DBS:
            return res
    ...
    def get_ec_from_ko(self, ko):
        if 'kegg' in SCRAPPABLE_DBS:
            ...
            if webpage:
                ...
                if enzyme_ec:
                    return enzyme_ec.group()[4:-1].split()
                else:
                    return []
        return []
...
if __name__ == '__main__':
    searcher = ProteinSearcher(search_mode={''}, politeness_timer=2)
    searcher.run_searcher('K01667', 'kegg_ko')
