...
from unifuncnet.fetchers.gene_fetchers.gene_fetcher_metacyc import GeneFetcherMetacyc
from unifuncnet.fetchers.gene_fetchers.gene_fetcher_hmdb import GeneFetcherHmdb
from unifuncnet.fetchers.gene_fetchers.gene_fetcher_kegg import GeneFetcherKegg


class GeneSearcher(GlobalSearcher):
    def __init__(self,
                 memory_storage=None,
                 search_mode='',
                 db_name=None,
    ...
    def select_fetcher(self, db, query_id, extra_args):
        if db in SCRAPPABLE_DBS and not self.check_already_searched_memory(db, query_id):
            if db == 'metacyc':
                return GeneFetcherMetacyc(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif 'kegg' in db:
                return GeneFetcherKegg(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif db == 'hmdb':
                return GeneFetcherHmdb(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            else:
            ...
            if bio_db == 'uniprot':
                for db in ['metacyc', 'hmdb', 'kegg']:
                    if db in SCRAPPABLE_DBS:
                        ...
                        if db_id:
                            args_to_search.append([db, db_id])
            else:
    ...
    def get_db_id_from_uniprot(self, uniprot_id, db):
        ...
        if db == 'hmdb':
            return self.get_db_id_from_uniprot_hmdb(uniprot_id)
        if db == 'kegg':
            return self.get_db_id_from_uniprot_kegg(uniprot_id)
    ...
    def get_db_id_from_uniprot_kegg(self, uniprot_id):
        if uniprot_id:
            # using kegg api
            webpage = self.get_with_fetcher(api_kegg=True,
                                            url='uniprot:' + uniprot_id,
                                            type_search='conv',
                                            database='genes')
            if webpage:
...
if __name__ == '__main__':
    searcher = GeneSearcher(search_mode='')
