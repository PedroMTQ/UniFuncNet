from unifuncnet.Utils.web_connector import WebConnector
...
class MemoryKeeper():
    ...
    def setup_memory_lists(self):
        self.reactions = ItemSet()
        self.proteins = ItemSet()
        self.genes = ItemSet()
        self.compounds = ItemSet()
    ...
    def initialize_fetcher(self, url_or_db):
        if not url_or_db:
            self.fetcher_others = WebConnector(test=self.fetcher_test,
                                                omit_error_messages=self.fetcher_omit_error_message,
            ...
            self.fetcher_kegg = WebConnector(test=self.fetcher_test,
                                              omit_error_messages=self.fetcher_omit_error_messages,
                                              politeness_timer=self.politeness_timer)
            return self.fetcher_kegg

        elif 'hmdb' in url_or_db.lower():
            self.fetcher_hmdb = WebConnector(test=self.fetcher_test,
                                              omit_error_messages=self.fetcher_omit_error_messages,
                                              politeness_timer=self.politeness_timer)
            ...
            self.fetcher_ncbi = WebConnector(test=self.fetcher_test,
                                              omit_error_messages=self.fetcher_omit_error_messages,
                                              politeness_timer=self.politeness_timer)
            return self.fetcher_ncbi
        elif 'uniprot' in url_or_db.lower():
            self.fetcher_uniprot = WebConnector(test=self.fetcher_test,
                                                 omit_error_messages=self.fetcher_omit_error_messages,
                                                 politeness_timer=self.politeness_timer)
    ...
    def get_db_fetcher(self, url_or_db=None):
        if not url_or_db:
            ...
            fetcher = self.fetcher_kegg
        elif 'hmdb' in url_or_db.lower():
            fetcher = self.fetcher_hmdb
        elif 'ncbi' in url_or_db.lower():
            fetcher = self.fetcher_ncbi
        elif 'uniprot' in url_or_db.lower():
            fetcher = self.fetcher_uniprot
        else:
            fetcher = self.fetcher_others
    ...
    def get_with_fetcher(self, url, api_kegg=False, data=None,
        ...
        if api_kegg:
            res = self.get_db_fetcher('kegg').api_kegg(to_search=url, database=database, api_type=type_search,
                                                       kegg_option=kegg_option)
        else:
    ...
    def get_biological_instance(self, bio_type, bio_query, bio_db=None):
        ...
        for bio_entity in bio_list:
            if bio_db:
                if bio_type == 'proteins':
                    temp_bio_entity = Protein({bio_db: bio_query})
                elif bio_type == 'genes':
                    temp_bio_entity = Gene({bio_db: bio_query})
                elif bio_type == 'reactions':
                    temp_bio_entity = Reaction({bio_db: bio_query})
                elif bio_type == 'compounds':
                    temp_bio_entity = Compound({bio_db: bio_query})

                if bio_entity.is_match_instances(temp_bio_entity, threshold_for_match=1):
                    return bio_entity
            else:
                if bio_entity.is_match_instances(bio_query):
                    return bio_entity
        return None
