...
from unifuncnet.fetchers.reaction_fetchers.reaction_fetcher_metacyc import ReactionFetcherMetacyc
from unifuncnet.fetchers.reaction_fetchers.reaction_fetcher_hmdb import ReactionFetcherHmdb
from unifuncnet.fetchers.reaction_fetchers.reaction_fetcher_kegg import ReactionFetcherKegg
from unifuncnet.fetchers.reaction_fetchers.reaction_fetcher_rhea import ReactionFetcherRhea


class ReactionSearcher(GlobalSearcher):
    def __init__(self,
                 memory_storage=None,
                 search_mode='',
                 db_name=None,
    ...
    def select_fetcher(self, db, query_id, extra_args):
        if db in SCRAPPABLE_DBS and not self.check_already_searched_memory(db, query_id):
            if db == 'metacyc':
                return ReactionFetcherMetacyc(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif 'kegg' in db:
                return ReactionFetcherKegg(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif db == 'hmdb':
                return ReactionFetcherHmdb(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            elif db == 'rhea':
                return ReactionFetcherRhea(query_id, extra_args=extra_args, memory_storage=self.memory_storage)
            else:
    ...
    def add_to_args_to_search(self, reaction_instance_or_list, args_to_search):
        if not isinstance(reaction_instance_or_list, list):
            reaction_instance_or_list = [reaction_instance_or_list]
        for reaction_instance in reaction_instance_or_list:
            if reaction_instance:
                ...
                for id_to_add in rhea_ids:
                    if id_to_add and not self.check_already_searched_memory(dict_input_key='rhea',
                                                                            dict_input_value=id_to_add):
                        args_to_search.append(['rhea', id_to_add])


if __name__ == '__main__':
    searcher = ReactionSearcher(search_mode={'rc'})
