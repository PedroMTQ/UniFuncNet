...
from unifuncnet.Utils.util import (SCRAPPABLE_DBS,
                                   set_scrappable_dbs,
                                   VALID_DIRECTIONS,
                                   print_version,
                                   UniFuncNet_FOLDER,
...
def run_test():
    ...
    searcher = ProteinSearcher(search_mode={'pr'}, output_folder=output_folder, politeness_timer=2)
    searcher.run_searcher('1.1.1.178', 'enzyme_ec')
    searcher.output_results()


def argv_gsmm_expansion_function():
    from workflows.gsmm_expansion.gsmm_expansion import GsmmExpansion
    ...
    parser.add_argument('-oc', '--only_connected', action='store_true',
                        help='[optional]\tExpand network with only nodes that are connected to the original network (this is off by default)')
    ...
    if input_folder and output_folder:
        GsmmExpansion(input_folder=input_folder,
                      output_folder=output_folder,
                      metacyc_ref=metacyc_ref,
                      database=database,
...
def argv_compounds_to_organism_function():
    from workflows.compounds_to_organisms_mapping.compounds_to_organisms_mapping import CompoundsToOrganismsMapping
    ...
    if os.path.exists(input_samples) and os.path.exists(metabolites) and output_folder and metacyc_ref:
        CompoundsToOrganismsMapping(input_samples=input_samples, metabolites=metabolites,
...
def argv_neo4j_function():
    from workflows.unifuncnet_neo4j_connector.unifuncnet_neo4j_connector import UniFuncNetNeo4jConnector
    print('Executing command:\n', ' '.join(sys.argv))
    parser = argparse.ArgumentParser(
    ...
    parser.add_argument('-i', '--input',
                        help='[required]\tInput folder if creating the Neo4j database or consensus_annotation.tsv if exporting network')
    ...
    neo4j_driver = UniFuncNetNeo4jConnector(username, password, db_name=database_name, uri=uri)
    try:
...
def argv_input_generator_function():
    from workflows.input_generator.input_generator import InputGenerator
    print('Executing command:\n', ' '.join(sys.argv))
    parser = argparse.ArgumentParser(
    ...
    parser.add_argument('-ko', '--ko_json',
                        help='[required]\tKO json from https://www.genome.jp/kegg-bin/show_brite?ko00001.keg')
    parser.add_argument('-db', '--database', help='[optional]\tDatabase to generate input. <all> by default',
                        choices=['all', 'ko'])
    ...
    s = InputGenerator(output_path=output_path)
    if database == 'ko':
...
def run_searcher(target_path, output_folder, politeness_timer, cpds_to_ignore={}):
    ...
    gene_searcher = GeneSearcher(output_folder=output_folder, politeness_timer=politeness_timer)
        ...
        while line:
            ...
            if line:
                ...
                if len(line_split) < 3:
                    ...
                    search_mode = set(search_mode)
                    instance_type = instance_type.lower()
                    if instance_type == 'gene':
                        searcher = gene_searcher
                    elif instance_type == 'protein':
                        searcher = protein_searcher
                    elif instance_type == 'compound':
                        searcher = compound_searcher
                    elif instance_type == 'reaction':
                        searcher = reaction_searcher

                    set_search_mode([gene_searcher, protein_searcher, compound_searcher, reaction_searcher],
                                    search_mode)
                    set_kegg_org_codes([gene_searcher, protein_searcher, compound_searcher, reaction_searcher],
                                       kegg_org_codes)
...
def main():
    if '--example' in sys.argv:
        ...
        parser.add_argument('-o', '--output_folder',
                            help='[required]\tOutput folder path for the information collected from the different sources')
        parser.add_argument('-db', '--databases',
                            help='[optional]\tComma separated list of databases to search for information in.')
        parser.add_argument('-ign_cpd', '--ignore_cpd', action='store_true',
                            help=f'[optional]\tWhether to ignore the list of compounds available in {cpd_to_ignore_path}')
        parser.add_argument('-pt', '--politeness_timer',
                            help='[optional]\tTime (seconds) between requests. Default is 10. Please be careful not to overleaf the corresponding databases, you might get blocked from doing future requests.')
