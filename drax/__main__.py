...
def argv_neo4j_function():
    from Workflows.DRAX_Neo4j_Connector.DRAX_Neo4j_Connector import DRAX_Neo4j_Connector
    print('Executing command:\n', ' '.join(sys.argv))
    parser = argparse.ArgumentParser(
    ...
    parser.add_argument('-pw', '--password', help='[optional]\tPassword, <drax_neo4j> by default')
    ...
    neo4j_driver = DRAX_Neo4j_Connector(username, password, db_name=database_name, uri=uri)
...
def argv_input_generator_function():
    ...
    print('Executing command:\n', ' '.join(sys.argv))
    parser = argparse.ArgumentParser(
        description='This workflow creates a DRAX input tsv with Rhea reaction IDs, Metacyc protein IDs, KOs, and ECs\n',
        formatter_class=argparse.RawTextHelpFormatter)
    #just a placeholder
    parser.add_argument('workflow')
