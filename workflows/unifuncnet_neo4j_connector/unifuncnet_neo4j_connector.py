...
class UniFuncNetNeo4jConnector:
    ...
    def create_node_labels(self, unifuncnet_output_folder):
        unifuncnet_output_files = os.listdir(unifuncnet_output_folder)
        # this is for the unifuncnet neo4j db, a general db wont have these
        exceptional_labels = ['_connected', 'reaction_compounds', 'has_subunits', 'in_complex', 'stoichiometry']
        self.node_labels = {}
        self.main_labels = set()
        all_indexes = set()
        for unifuncnet_output_file in unifuncnet_output_files:
            node_type = unifuncnet_output_file.split('.')[0]
            # node_type for Proteins.tsv becomes Protein
            node_type = f'{node_type[0].upper()}{node_type[1:]}'
            node_type = node_type.strip('s')
            unifuncnet_output_file_path = f'{unifuncnet_output_folder}{unifuncnet_output_file}'
            generator_unifuncnet_output = self.yield_tsv(unifuncnet_output_file_path)
            for internal_id, node_info in generator_unifuncnet_output:
    ...
    def create_indexes(self, all_indexes):
        ...
        for i in self.main_labels:
            command = f' INDEX ON :{i}(unifuncnet_id)'
            res.append(command)
        return res
    ...
    def create_constraints(self, all_indexes):
        ...
        for i in all_indexes:
            if i not in self.main_labels:
                ...
                res.append(command)
        for i in self.main_labels:
            command = f' CONSTRAINT {i} FOR (c:{i}) REQUIRE c.unifuncnet_id IS UNIQUE '
            res.append(command)

        return res

    def add_constraints(self, all_indexes):
        active_constraints = [i['description'] for i in
                              self.run_command_neo4j('CALL db.constraints() YIELD description RETURN description')]
    ...
    def yield_tsv(self, unifuncnet_tsv):
        try:
    ...
    def generate_chunks_unifuncnet_output(self, unifuncnet_wielder, node_type):
        step = self.insert_step
        temp = []
        for node_id, node_info in unifuncnet_wielder:
            sub_nodes = self.process_node_info(node_info)
            node_data = {'unifuncnet_id': node_id, 'node_type': node_type, 'node_data': sub_nodes}
    ...
    def create_nodes_unifuncnet(self, unifuncnet_output_folder):
        unifuncnet_output_files = os.listdir(unifuncnet_output_folder)
        for unifuncnet_output_file in unifuncnet_output_files:
            node_type = unifuncnet_output_file.split('.')[0]
            node_type = f'{node_type[0].upper()}{node_type[1:]}'
            ...
            unifuncnet_output_file_path = f'{unifuncnet_output_folder}{unifuncnet_output_file}'
            generator_unifuncnet_output = self.yield_tsv(unifuncnet_output_file_path)
            generator_insert = self.generate_chunks_unifuncnet_output(generator_unifuncnet_output, node_type)
            for chunk in generator_insert:
                if chunk:
                    if node_type == 'Reaction':
                        command_to_run = f'WITH $batch as chunk UNWIND chunk as chunk_data ' \
                                         f'CREATE (n:{node_type}  {{node_type: chunk_data.node_type ,unifuncnet_id: chunk_data.unifuncnet_id, reversible: chunk_data.reversible }}) ' \
                                         f'WITH n, chunk_data.node_data as node_data '
                    else:
                        command_to_run = f'WITH $batch as chunk UNWIND chunk as chunk_data ' \
                                         f'CREATE (n:{node_type}  {{node_type: chunk_data.node_type ,unifuncnet_id: chunk_data.unifuncnet_id}} ) ' \
                                         f'WITH n, chunk_data.node_data as node_data '
    ...
    def get_reaction_compounds_neo4j(self, unifuncnet_id, reaction_str, stoichiometry):
        res = []
        ...
        for cpd_id in left_side:
            ...
            item = {'start_unifuncnet_id': unifuncnet_id, 'end_unifuncnet_id': cpd_id,
    ...
    def get_connections(self, generator_unifuncnet_output):
        res = {}
        ...
        for unifuncnet_id, node_info in generator_unifuncnet_output:
                    ...
                    for connection_id in node_info[connection_type]:
                        ...
                        if connection_type == 'reaction_compounds':
                            if 'stoichiometry' in node_info:
                                ...
                                reaction_compounds = self.get_reaction_compounds_neo4j(unifuncnet_id, connection_id,
                                                                                       stoichiometry)
                            ...
                            item = {'start_unifuncnet_id': unifuncnet_id, 'end_unifuncnet_id': connection_id}
    ...
    def connect_nodes_unifuncnet(self, unifuncnet_output_folder):
        unifuncnet_output_files = os.listdir(unifuncnet_output_folder)
        for unifuncnet_output_file in unifuncnet_output_files:
            node_type = unifuncnet_output_file.split('.')[0]
            node_type = f'{node_type[0].upper()}{node_type[1:]}'
            ...
            unifuncnet_output_file_path = f'{unifuncnet_output_folder}{unifuncnet_output_file}'
            generator_unifuncnet_output = self.yield_tsv(unifuncnet_output_file_path)
            all_connections = self.get_connections(generator_unifuncnet_output)
                ...
                for chunk in self.yield_list(all_connections[connection_label]):
                    if chunk:
                        if connection_label in ['IS_SUBSTRATE', 'IS_PRODUCT']:
                            command_to_run = f'WITH $batch as chunk UNWIND chunk as chunk_data ' \
                                             f'MATCH (n1:{node_type}  {{unifuncnet_id: chunk_data.start_unifuncnet_id}}) MATCH (n2  {{unifuncnet_id: chunk_data.end_unifuncnet_id}}) ' \
    ...
    def export_unifuncnet_to_neo4j(self, unifuncnet_output_folder):
        self.start_time = time.time()
        if not unifuncnet_output_folder.endswith(SPLITTER):
            unifuncnet_output_folder += SPLITTER
        print('Creating node labels')
        self.create_node_labels(unifuncnet_output_folder)
    ...
    def parse_fetch_annotations_results(self, db, fetch_results, res):
        for i in fetch_results:
            ...
            db_id = i['db_id']
            unifuncnet_id = i['unifuncnet_id']
            # if db_id not in res[db]: res[db][db_id]=set()
            # res[db][db_id].add(unifuncnet_id)
            res.add(unifuncnet_id)
    ...
    def parse_fetch_node_results(self, fetch_results, res):
        for i in fetch_results:
            ...
            n1_unifuncnet_id = i['n1']['unifuncnet_id']
            n2_node_type = i['n2'].get('node_type')
            n2_unifuncnet_id = i['n2'].get('unifuncnet_id')
            if not n2_unifuncnet_id:
                n2_unifuncnet_id = i['n2'].get('node_info')
            ...
            if n1_unifuncnet_id not in res:
                res[n1_unifuncnet_id] = {}

            if relationship not in res[n1_unifuncnet_id]:
                if relationship in ['IS_SUBSTRATE', 'IS_PRODUCT']:
                    if n1_node_type != 'Compound':
                        res[n1_unifuncnet_id][relationship] = []
                else:
                    res[n1_unifuncnet_id][relationship] = set()
            if relationship in ['IS_SUBSTRATE', 'IS_PRODUCT']:
                if n1_node_type != 'Compound':
                    res[n1_unifuncnet_id][relationship].append([stoichiometry, n2_unifuncnet_id])
            else:
                res[n1_unifuncnet_id][relationship].add(n2_unifuncnet_id)

    def get_nodes_info(self, node_type, nodes_list):
        unwind_items = []
        res = {}
        for unifuncnet_id in nodes_list:
            unwind_items.append({'unifuncnet_id': unifuncnet_id})
        for chunk in self.yield_list(unwind_items):
            command_to_run = f'WITH $batch as chunk UNWIND chunk as chunk_data ' \
                             f'MATCH (n1:{node_type} {{unifuncnet_id: chunk_data.unifuncnet_id}})-[r]-(n2) RETURN n1,n2,r,r.stoichiometry'
    ...
    def export_neo4j_to_tsv(self, output_tsv_folder, genes_info, proteins_info, reactions_info, compounds_info):
        ...
        with open(f'{output_tsv_folder}Graph.sif', 'w+') as file:
            ...
            for source_type, info_dict in output_tsvs:
                for unifuncnet_id in info_dict:
                    line_info = info_dict[unifuncnet_id]
                    for db in line_info:
                        if db == 'reaction_compounds':
                            for target_id in line_info[db].split():
                                try:
                                    int(target_id)
                                    target_type = 'c'
                                    line = f'{unifuncnet_id}\t{source_type}{target_type}\t{target_id}\n'
                                    file.write(line)
                                except:
                                    pass
    ...
    def get_network_from_annotations(self, input_tsv, output_tsv_folder):
        ...
        for db in input_annotations:
            for chunk in self.yield_list(input_annotations[db]):
                command_to_run = f'WITH $batch as chunk UNWIND chunk as chunk_data ' \
                                 f'MATCH (n:{db} {{node_info: chunk_data.node_info}})<--(n2) RETURN chunk_data.node_info as db_id,n2.unifuncnet_id as unifuncnet_id'
                fetch_results = self.run_command_neo4j(command_to_run=command_to_run, batch=chunk)
